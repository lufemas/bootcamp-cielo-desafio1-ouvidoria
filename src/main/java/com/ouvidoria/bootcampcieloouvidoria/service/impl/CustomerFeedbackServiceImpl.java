...
public class CustomerFeedbackServiceImpl implements CustomerFeedbackService {
    ...
    public String sendFeedback(CustomerFeedbackRequestDTO feedback) {
        if (!feedback.getType().equals(FeedbackType.SUGGESTION) && !feedback.getType().equals(FeedbackType.PRAISE) && !feedback.getType().equals(FeedbackType.CRITICISM) && !feedback.getType().equals(FeedbackType.NOTIFICATION)) {
    ...
    public List<CustomerFeedbackResponseDTO> getMessage() {
        String suggestionQueueUrl = amazonSQSClient.getQueueUrl("Suggestion.fifo").getQueueUrl();
        AmazonSQSClient sqsClient = (AmazonSQSClient) amazonSQSClient.builder().withRegion(Regions.US_EAST_1).build();
